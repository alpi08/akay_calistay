(() => {
  ...
  const translations = {
    tr: {
      ...
      hero:{eyebrow:'Perspektif / Aydınlanma / Buluşma',titleA:'Fikrin',titleB:'ışığı',titleC:'burada.',description:'Genç zihinlerin dünyayı yeniden okumak, anlamlandırmak ve dönüştürmek için bir araya geldiği alan.',cta:'Çalıştayı keşfet',secondary:'Manifestoyu oku',meta:'15 EYLÜL 2026'},
      about:{eyebrow:'Çalıştayın özü',title:'Bir fikrin<br><em>perspektifi</em> değiştirir.',lead:'Akay, cevaplardan çok daha iyi soruların peşinde olan genç bir düşünce alanıdır.',missionLabel:'MİSYON',missionTitle:'Bilgiyi paylaş,<br>ufku genişlet.',missionText:'Akay Çalıştayı olarak misyonumuz; gençlerin hızla gelişen ve değişen dünyaya dair farkındalıklarını artırırken, fikirlerini özgürce ifade edebilecekleri güçlü bir bilgi paylaşım alanı sunmaktır. Akademik olarak geliştirici ve günlük hayatı etkileyen konularla çok yönlü bakış açıları kazandırmayı hedefliyoruz.',visionLabel:'VİZYON',visionTitle:'Köklere bak,<br>geleceği kur.',visionText:'Akay Perspektif ve Aydınlanma Çalıştayı ekibi olarak gençlerin Türk kültürünü ve kökenini, dünya tarihini, farklı kültürleri ve dilleri anlayarak entelektüel donanımı ve hitabet becerisi yüksek bireyler olmalarına katkı sunmayı amaçlıyoruz. Nihai hedefimiz, bilgiyi ve aydınlık zihinleri insanlığa somut faydaya dönüştürmektir.'},
      event:{eyebrow:'Deneyimin koordinatları',title:'Buluşmanın<br><em>haritası.</em>',lead:'Gerçek ayrıntılar açıklandığında bu alan güncellenecek. Şimdilik tarih, fikrin merkezinde duruyor.',dateLabel:'TARİH',location:'Konum',fee:'Katılım ücreti',duration:'Süre',format:'Format',formatValue:'Çalıştay'},
      countdown:{eyebrow:'Başlamasına kalan',days:'Gün',hours:'Saat',minutes:'Dakika',seconds:'Saniye',started:'Etkinlik başladı.'},
      flow:{eyebrow:'Zamanın akışı',title:'Program,<br><em>yakında.</em>',lead:'Çalıştayın tam programı açıklandığında bu akış, her oturumu kendi ritmiyle taşıyacak.',empty:'Program yakında açıklanacaktır.',support:'Zaman / oturum / komite / konuşmacı bilgileri burada yer alacak.'},
      committees:{eyebrow:'Düşünce masaları',title:'Soruların<br><em>komiteleri.</em>',lead:'Her masa, farklı bir bakışın karşılaşma noktası. İçerik ve gündem yakında duyurulacak.',empty:'Komiteler ve gündem maddeleri yakında açıklanacaktır.',cta:'İndeksi aç'},
      sponsors:{eyebrow:'Birlikte mümkün',title:'Bu deneyimin<br><em>ortakları.</em>',lead:'Fikrin büyümesi, doğru insanların ve markaların yan yana gelmesiyle mümkün olur.',vertex:'Artificial Intelligence sponsor',banna:'IT sponsor',cord:'Social media visuals and graphic design sponsor'},
      team:{eyebrow:'Koordinasyon',title:'İnsanlar,<br><em>fikrin taşıyıcıları.</em>',coordinator:'Genel Koordinatör'},
  ...
  function initSegmentViews() {
    ...
    $$('a[href^="#"]').forEach(link => { const route = link.getAttribute('href').slice(1); if (route && route !== 'home') { link.target = '_blank'; link.rel = 'noopener noreferrer'; } });
    const progress = document.createElement('div'); progress.className = 'segment-progress'; document.body.appendChild(progress);
    const groups = { home:['home'], about:['about'], event:['event'], location:['event'], fee:['event'], duration:['event'], format:['event'], flow:['flow'], committees:['committees'], sponsors:['sponsors'], contact:['team','contact'] };
    ...
    document.addEventListener('click', event => { const link = event.target.closest('a[href^="#"]'); if (!link) return; const target = link.getAttribute('href').slice(1); if (!groups[target]) return; if (link.target === '_blank') { $('.mobile-menu')?.classList.remove('open'); $('.menu-toggle')?.setAttribute('aria-expanded','false'); return; } event.preventDefault(); if (window.location.hash !== `#${target}`) window.location.hash = target; else render(); $('.mobile-menu')?.classList.remove('open'); $('.menu-toggle')?.setAttribute('aria-expanded','false'); }, true);
