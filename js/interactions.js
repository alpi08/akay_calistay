...
document.addEventListener("DOMContentLoaded", function(){
  ...
  // hero video: her cihazda oynasın (mobil autoplay garantili), ekran dışında dursun
  const v = document.getElementById("heroVideo");
  if(v){
    v.muted = true; // iOS autoplay için property şart
    ...
    v.addEventListener("playing", function(){ playOK = true; });
    // kaynak hatasında diğer sürüme düş (örn. mobil dosya engellenirse masaüstü)
    v.addEventListener("error", function(){
      var srcs = v.querySelectorAll("source");
      var failed = v.currentSrc || "";
      for(var i = 0; i < srcs.length; i++){
        if(failed.indexOf(srcs[i].getAttribute("src")) !== -1 && i + 1 < srcs.length){
          srcs[i].removeAttribute("media");
          v.load(); tryPlay(); return;
        }
      }
      v.style.display = "none"; // son çare: animasyonlu poster görünür
    }, true);
    if(window.matchMedia("(prefers-reduced-motion: reduce)").matches){ v.pause(); v.removeAttribute("autoplay"); }
      ...
      // mobil tarayıcı autoplay'i engellerse ilk dokunuşta / sekmeye dönüşte başlat
      document.addEventListener("touchend", tryPlay, {once:true, passive:true});
      document.addEventListener("click", tryPlay, {once:true});
      document.addEventListener("visibilitychange", function(){ if(!document.hidden) tryPlay(); });
      // 6sn'de hâlâ oynamıyorsa bir kez daha zorla
      setTimeout(function(){ if(v.paused && !playOK) tryPlay(); }, 6000);
