...
document.addEventListener("DOMContentLoaded", function(){
  ...
  if(v){
    ...
    // tarayıcı kaynak zincirini kendi dener (mobil → masaüstü → MOV);
    // hepsi biterse gizle, animasyonlu poster görünür
    var errCount = 0;
    var srcCount = v.querySelectorAll("source").length;
    v.addEventListener("error", function(){
      errCount++;
      if(errCount >= srcCount) v.style.display = "none";
    }, true);
